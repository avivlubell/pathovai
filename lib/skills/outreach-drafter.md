...
4. **Institutional Knowledge Gap** [internal id: `FOUNDER_LED_CEILING`] — Use when the patterns and judgment driving wins are concentrated in the founder, exec team, or clinical/domain SMEs, and the team can't run the same play. The issue is not burnout — it's that knowledge has never been extracted, codified, AND made accessible. Trigger signals: early wins came through the founder's personal network; knowledge "lives in [founder/exec/SME] heads"; no documentation a new hire could use; a sales hire failed because they inherited a job description instead of a playbook; deals stall when specific people aren't in the room. The fix is extracting what's in those heads and making it accessible to the team — not removing anyone from the process. Founder-led selling itself isn't the bottleneck; the uncodified, inaccessible playbook is.
